...
const Recorder = () => {
  ...
  // Add a new state variable for payment required
  ...
  useEffect(() => {
    const handleBeforeUnload = (event) => {
      if (isUploading) {
        // This is needed for browser's standard dialog
        event.preventDefault();
        event.returnValue = '';

        // Attempt to use chrome extension API to get more control
        chrome.runtime.sendMessage({
          type: "show-upload-warning",
          message: "Closing during uploads might result in losing or corruption of the video."
        });
  ...
  async function startRecording() {
    ...
    try {
      ...
      const errMessage = err.stack + " | message: " + err.message;
      chrome.runtime.sendMessage({
        type: "recording-error",
        error: "stream-error",
        why: errMessage,
  ...
  async function startStream(data, id, options, permissions, permissions2) {
    // Get quality value
    let { qualityValue } = await chrome.storage.local.get(["qualityValue"]);
    console.log("Quality value: ", qualityValue);
    // Consider 720p as the lowest possible quality
    if (!qualityValue) {
      qualityValue = "1080p";
    } else if (
  ...
  async function startStreaming(data) {
    ...
    try {
      if (data.recordingType === "camera") {
        ...
        let captureTypes = ["tab", "window", "screen"];
  ...
  // Update the onMessage function to handle payment-required messages
  const onMessage = useCallback(
    (request, sender, sendResponse) => {
      ...
      if (request.type === "streaming-data") {
        ...
        dismissRecording();
      } else if (request.type === "payment-required") {
        // Set payment required state to true
        setPaymentRequired(true);
      } else if (request.type === "upload-started") {
        setIsUploading(true);
      } else if (request.type === "upload-completed" || request.type === "upload-failed") {
  ...
  return (
    <div className="wrap">
      <div className="middle-area">
        ...
        <div className="title">
          {paymentRequired
            ? chrome.i18n.getMessage("paymentRequiredTitle") || "Payment Required"
            : !started
        ...
        <div className="subtitle">
          {paymentRequired
            ? chrome.i18n.getMessage("paymentRequiredDescription") || "Please purchase or upgrade your plan to continue"
        ...
        {paymentRequired && (
          ...
          >
            {chrome.i18n.getMessage("upgradeNowLabel") || "Upgrade Now"}
          </div>
        )}
      </div>
      {/* FEATURE_DISABLED {!isTab.current && !started && <Warning />} */}
