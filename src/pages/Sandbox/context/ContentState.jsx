...
const ContentState = (props) => {
  ...
  useEffect(() => {
    ...
    setContentState((prevState) => ({
      ...prevState,
      title: `Trupeer.ai video - ${formattedDate}`,
    }));
  }, []);
  ...
  const reconstructVideo = async () => {
    console.log("Recording complete, reconstructing video");
      ...
      if (recordingDuration > 0 && recordingDuration !== null) {
        if (!isWindows10) {
          ...
          if (
            ...
            console.log("Recording complete, video reconstructed");
            chrome.runtime.sendMessage({ type: "recording-complete" });
            return;
          }
  ...
  const handleBatch = async (chunks, sendResponse) => {
    ...
    await Promise.all(
      ...
      .catch((error) => {
        ...
        // Handle error scenario, possible notify sender of failure
      });
  ...
  // FEATURE_DISABLED = const requestDownload = async (url, type) => {
  //   const ext = type;
