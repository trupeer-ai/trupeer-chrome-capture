...
const Backup = () => {
  ...
  const initLocalDirectory = async (directoryHandle, prompt = true) => {
    ...
    if (permissions) {
      let videoTitle = `Trupeer.ai video - ${new Date().toLocaleString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
        hour: "numeric",
      ...
      alert(
        "Failed to set up local backup. Reach out to us at support@trupeer-ai.io for more help. You can still record your screen."
      );
  ...
  const directoryPicker = async (prompt = true) => {
    ...
    // Check if the selected directory is the "TrupeerAI recordings" folder
    if (directoryPicker.name === "TrupeerAI Recordings") {
      // Use the selected directory directly
      directoryHandle = directoryPicker;
    } else {
      // If not, create the "TrupeerAI recordings" folder within it
      directoryHandle = await directoryPicker.getDirectoryHandle(
        "TrupeerAI Recordings",
        { create: true }
      );
    }
  ...
  const localSaving = async (prompt = true) => {
    ...
    if ("showDirectoryPicker" in window) {
      ...
      alert(
        "Your browser doesn't support local backups. Reach out to us at support@trupeer-ai.io for more help. You can still record your screen."
      );
      chrome.storage.local.set({ backup: false });
      chrome.runtime.sendMessage({
        type: "backup-created",
        ...
        <div className="setupContainer">
          ...
          <div className="setupText">
            ...
            <div className="setupDescription">
              ...
              <a
                href="https://help.trupeer-ai.io/getting-started/77KizPC8MHVGfpKpqdux9D/what-are-backups-how-can-i-set-them-up/waYArvSwybZkJKKDdMXw1o"
                target="_blank"
              >
