  ...
  try {
    const tab = await new Promise((resolve, reject) => {
      chrome.tabs.get(tabId, (tab) => {
        if (chrome.runtime.lastError) {
          // ErrorLogs: console.log("Error fetching tab:", chrome.runtime.lastError.message);
          resolve();
        } else {
          resolve(tab);
        }
    ...
    return new Promise((resolve, reject) => {
      chrome.tabs.sendMessage(tab.id, message, (response) => {
        if (chrome.runtime.lastError) {
          // ErrorLogs: console.log(
          //   "Error sending message:",
          //   chrome.runtime.lastError.message
          // );
          resolve();
        } else {
          // ErrorLogs: console.log("Received response:", response);
          responseCallback ? responseCallback(response) : resolve(response);
        }
      });
    });
  } catch (error) {
    // ErrorLogs: console.error("Error sending message to tab:", error);
