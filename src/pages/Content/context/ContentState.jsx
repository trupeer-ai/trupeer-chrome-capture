...
const ContentState = (props) => {
  ...
  const [URL, setURL] = useState(
    "https://help.trupeer-ai.io/getting-started/77KizPC8MHVGfpKpqdux9D/why-does-trupeer-ai-ask-for-permissions/9AAE8zJ6iiUtCAtjn4SUT1"
  );
  const [URL2, setURL2] = useState(
    "https://help.trupeer-ai.io/troubleshooting/9Jy5RGjNrBB42hqUdREQ7W/how-to-grant-trupeer-ai-permission-to-record-your-camera-and-microphone/x6U69TnrbMjy5CQ96Er2E9"
  );
    ...
    if (!locale.includes("en")) {
      setURL(
        "https://translate.google.com/translate?sl=en&tl=" +
          locale +
          "&u=https://help.trupeer-ai.io/getting-started/77KizPC8MHVGfpKpqdux9D/why-does-trupeer-ai-ask-for-permissions/9AAE8zJ6iiUtCAtjn4SUT1"
      );
      setURL2(
        "https://translate.google.com/translate?sl=en&tl=" +
          locale +
          "&u=https://help.trupeer-ai.io/troubleshooting/9Jy5RGjNrBB42hqUdREQ7W/how-to-grant-trupeer-ai-permission-to-record-your-camera-and-microphone/x6U69TnrbMjy5CQ96Er2E9"
      );
    }
  }, []);
      ...
      .then(async (data) => {
        ...
        if (data.quota < 524288000) {
          if (typeof contentStateRef.current.openModal === "function") {
            ...
            const locale = chrome.i18n.getMessage("@@ui_locale");
            let helpURL =
              "https://help.trupeer-ai.io/troubleshooting/9Jy5RGjNrBB42hqUdREQ7W/what-does-%E2%80%9Cmemory-limit-reached%E2%80%9D-mean-when-recording/8WkwHbt3puuXunYqQnyPcb";

            if (!locale.includes("en")) {
              helpURL =
                "https://translate.google.com/translate?sl=en&tl=" +
                locale +
                "&u=https://help.trupeer-ai.io/troubleshooting/9Jy5RGjNrBB42hqUdREQ7W/what-does-%E2%80%9Cmemory-limit-reached%E2%80%9D-mean-when-recording/8WkwHbt3puuXunYqQnyPcb";
            }
          ...
          // ExtraFeatures: // Check if recording a region & if a custom region is set
          // if (
          //   contentStateRef.current.recordingType === "region" &&
          //   contentStateRef.current.cropTarget
          // ) {
  ...
  const handleDevicePermissions = (data) => {
    if (data && data != undefined && data.success) {
      ...
      if (!contentStateRef.current.setDevices) {
        ...
        if (audioInput && Array.isArray(audioInput) && audioInput.length > 0) {
          setContentState((prevContentState) => ({
            ...prevContentState,
        ...
        if (videoInput && Array.isArray(videoInput) && videoInput.length > 0) {
          setContentState((prevContentState) => ({
            ...prevContentState,
        ...
        if (
          (audioInput && Array.isArray(audioInput) && audioInput.length > 0) ||
          (videoInput && Array.isArray(videoInput) && videoInput.length > 0)
        ) {
          setContentState((prevContentState) => ({
            ...prevContentState,
            setDevices: true,
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      if (event.data.type === "trupeer-ai-permissions") {
        handleDevicePermissions(event.data);
      } else if (event.data.type === "trupeer-ai-permissions-loaded") {
        setContentState((prevContentState) => ({
          ...prevContentState,
          permissionsLoaded: true,
        }));
      } else if (event.data.type === "permissions-not-possible") {
        chrome.runtime.sendMessage({
  ...
  // @IMPORTANT: REPLACING WITH NO PAUSES - WE ARE HANDLING IT MANUALLY IN TOOLBARWRAP.JSX
  // Count up every second
  useEffect(() => {
    if (contentState.recording && !contentState.alarm) {
      setTimer((timer) => timer + 1);
      const interval = setInterval(() => {
        setTimer((timer) => timer + 1);
      }, 1000);
  ...
  }, [contentState.recording, contentState.alarm]);
  ...
  const onMessage = useCallback(
    (request, sender, sendResponse) => {
      if (request.type === "time") {
        ...
        console.log("Stream error encountered: ", request.why);
        contentStateRef.current.openModal(
  ...
  useEffect(() => {
    const checkRAM = () => {
      ...
      if (
        ...
        chrome.storage.local.set({
          qualityValue: contentState.qualityValue,
        });
      }
    };
    checkRAM();
  ...
  useEffect(() => {
    ...
    elements.forEach((element) => {
      element.classList.add("trupeer-ai-scrollbar");
    });
        ...
        if (mutation.type === "childList") {
          ...
          addedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              node.classList.add("trupeer-ai-scrollbar");
            }
          });

          removedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              node.classList.remove("trupeer-ai-scrollbar");
            }
          });
        }
      }
    ...
    chrome.storage.local.get(
      ...
      (result) => {
        setContentState((prevContentState) => ({
          ...
          inputLanguage:
            result.inputLanguage !== undefined && result.inputLanguage !== null
              ? result.inputLanguage
              : prevContentState.inputLanguage,
          recording:
        ...
        if (
          result.inputLanguage === undefined ||
          result.inputLanguage === null ||
          result.inputLanguage === "english"
        ) {
          chrome.storage.local.set({ inputLanguage: "en" });
        }
  ...
  useEffect(() => {
    ...
    chrome.storage.local.get(["recording", "isPaused"], (result) => {
      if (result.recording) {
        setContentState((prev) => ({
          ...prev,
          recording: true,
          paused: result.isPaused || false,
        }));
      }
    });
  }, []);
