...
import { TempLogo, ProfilePic, Logo } from "../images/popup/images";
...
import AccessPopup from "./layout/AccessPopup";
...
const PopupContainer = (props) => {
  ...
  const [areCreditsExhausted, setAreCreditsExhausted] = useState(false);
  ...
  async function checkLogin() {
    ...
    if (
      !response.accessTokenValue ||
      !response.userInfo ||
      CheckTokenExpiry(response.accessTokenValue)
    ) {
      await chrome.storage.local.set({ accessTokenValue: "", userInfo: null });
      setOnboarding(true);
    } else {
      ...
      checkCredits(response.accessTokenValue);
    }
  }

  async function checkCredits() {
    chrome.runtime
      .sendMessage({ type: "check-user-credits" })
      .then((response) => {
        if (!response || response.creditsExhausted) {
          setAreCreditsExhausted(true);
        }
      });
  }
  ...
  useEffect(() => {
    checkLogin();
  }, []); // Call checkLogin at component mount
      ...
      >
        <div className="popup-container" ref={PopupRef}>
          ...
          <div className="popup-content">
            ...
            {!onboarding && !areCreditsExhausted && (
              <Tabs.Root
                ...
                >
                  ...
                  >
                    <a
                      ...
                      href={`${getFrontendBaseEndpoint()}/`}
                      target="_blank"
                    >
                      <img src={Logo} alt="Logo" />
                    </a>
                    {/* {chrome.i18n.getMessage("companyName")} */}
                  </Tabs.Trigger>
                </Tabs.List>
            ...
            {!onboarding && areCreditsExhausted && <AccessPopup />}
          </div>
        </div>
      </Rnd>
    </div>
